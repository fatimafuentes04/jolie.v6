<?php
require_once('../../entities/dto/estadopedido.php');
    ...
    if (isset($_SESSION['id_usuario'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'search':
                ...
                } elseif ($result['dataset'] = $estadopedido->searchRows($_POST['search'])) {
                    $result['status'] = 1;
            ...
            case 'create':
                $_POST = Validator::validateForm($_POST);
                if (!$estadopedido->setTalla($_POST['talla'])) {
            ...
            case 'update':
                $_POST = Validator::validateForm($_POST);
                if (!$estadopedido->setId($_POST['id'])) {
                    $result['exception'] = 'id de talla incorrecta';
                } elseif (!$data = $estadopedido->readOne()) {
                    $result['exception'] = 'talla inexistente';
                } elseif (!$estadopedido->setTalla($_POST['talla'])) {
